...
// Waffen
player addWeapon dm;
player addPrimaryWeaponItem dm_scope;
player addPrimaryWeaponItem dm_attachment1;
player addPrimaryWeaponItem dm_attachment2;
player addPrimaryWeaponItem dm_mags;
...
//	Primärwaffe
//------------------------------------------------------------------
for "_i" from 1 to dm_mags_count do {
	_fits = player canAddItemToVest dm_mags;
	if (_fits) then {
		player addItemToVest dm_mags;
	}
	else {
		_fits = false;
		_fits = player canAddItemToBackpack dm_mags;
		if (_fits) then {
			player addItemToBackpack dm_mags;
		}
		else {
			_fits = false;
			_fits = player canAddItemToUniform dm_mags;
			if (_fits) then {
				player addItemToUniform dm_mags;
			}
			else {
				_notloadeditems pushback dm_mags;
			};
		};
	};
};

for "_i" from 1 to dm_tracer_count do {
	_fits = player canAddItemToVest dm_tracer;
	if (_fits) then {
		player addItemToVest dm_tracer;
	}
	else {
		_fits = false;
		_fits = player canAddItemToBackpack dm_tracer;
		if (_fits) then {
			player addItemToBackpack dm_tracer;
		}
		else {
			_fits = false;
			_fits = player canAddItemToUniform dm_tracer;
			if (_fits) then {
				player addItemToUniform dm_tracer;
			}
			else {
				_notloadeditems pushback dm_tracer;
			};
		};
	};
};
