...
player setVariable ["loadout","Sierra_Sniper_G22"];
_notloadeditems = [];
...
// Waffen
player addWeapon sierra_sniper;
player addPrimaryWeaponItem sierra_sniper_scope;
player addPrimaryWeaponItem sierra_sniper_attachement1;
player addPrimaryWeaponItem sierra_sniper_attachement2;
player addPrimaryWeaponItem sierra_sniper_silencer;
player addPrimaryWeaponItem sierra_sniper_mag;
...
//	Primärwaffe
//------------------------------------------------------------------
for "_i" from 1 to sierra_sniper_mag_count do {
	_fits = player canAddItemToVest sierra_sniper_mag;
	if (_fits) then {
		player addItemToVest sierra_sniper_mag;
	}
	else {
		_fits = false;
		_fits = player canAddItemToBackpack sierra_sniper_mag;
		if (_fits) then {
			player addItemToBackpack sierra_sniper_mag;
		}
		else {
			_fits = false;
			_fits = player canAddItemToUniform sierra_sniper_mag;
			if (_fits) then {
				player addItemToUniform sierra_sniper_mag;
			}
			else {
				_notloadeditems pushback sierra_sniper_mag;
			};
		};
	};
};
